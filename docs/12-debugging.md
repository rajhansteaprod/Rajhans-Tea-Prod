...
docker logs rajhans-tea-mongo

# 3. Is replica set initialized?
docker exec rajhans-tea-mongo mongosh --eval "rs.status()"
...
docker exec rajhans-tea-backend env | grep JWT_ACCESS_SECRET
```
...
Debug:
docker logs rajhans-tea-backend | grep -i firebase
```
...
docker logs rajhans-tea-backend | grep "abc123"
```
...
docker logs rajhans-tea-backend --tail=50 --follow

# Enter a container shell
docker exec -it rajhans-tea-backend sh
docker exec -it rajhans-tea-mongo mongosh
