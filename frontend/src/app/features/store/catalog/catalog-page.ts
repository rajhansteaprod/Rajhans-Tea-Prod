...
export class CatalogPageComponent implements OnInit {
  ...
  constructor() {
    // Ratings are progressive-enhancement data for listing cards.
    // Keep them out of SSR/prerender so an older API cannot invalidate
    // an otherwise valid catalog page.
  ...
  ngOnInit(): void {
    // Variant facets are optional UI data and are not required for SSR/prerender.
    // Fetch them only in the browser so a temporarily older API cannot turn an
    // otherwise valid SEO route into a prerendered /404 redirect.
    if (isPlatformBrowser(this.platformId)) {
